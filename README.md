...
版本 1.1.6。离线 Windows 古典密码工具，交付为单个 `密码箱.exe`。
...
5. `?` 打开该工具的说明。`Ω` 设置 26 字符字母表，以及大小写、空白、标点、I/J 合并、变音符号。

「打开」可多选文本文件，也可把文件拖入窗口。多文件按文件名分段处理。
...
**单表替换破解**可锁定映射，如 `X=E` 或 `Ж=E`。语言 `ZH` 可恢复被单表替换的四位 Unicode 十六进制中文，以及无声调拼音。  
