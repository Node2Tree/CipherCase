...
namespace ClassicalCipherToolbox
{
    internal static class ToolDocumentation
    {
        internal static string GetSummary(string name)
        {
            switch (name)
            {
                case "格雷码": return "二进制与反射格雷码双向转换；每组独立处理并保留前导零。";
                case "进制换算": return "在 2–36 进制之间精确转换整数、小数、分数、循环小数和十进制科学计数法。";
                case "数据类型编码": return "在数值与计算机位模式之间转换，覆盖定宽整数、IEEE 754 浮点、bfloat16、.NET Decimal 和 Q 定点数。";
                case "碱基转氨基酸": return "按标准遗传密码表翻译 DNA/RNA，支持六种阅读框、FASTA 和模糊碱基。";
                ...
                case "中文电码加密": return "在四位中文电码上加减 0–9999 的密钥数；破解枚举密钥并用现代汉语或文言文模型排序。";
                case "中文语言评分": return "分别以现代汉语和文言文常用字、词组评价中文文本，并可自动选择更接近的模型。";
                case "反切码": return "用声母代表字与韵母代表字组成可反查的普通话反切对，保留数字声调。";
                case "中文代码本": return "用可导入的中文词组代码本加解密，编码时优先匹配最长词组。";
                case "中文隐写分析": return "自动枚举行首、行尾、固定间隔、逐列、蛇形、对角线和螺旋读取路径，并按中文模型排序。";
                ...
                case "旗语": return "显示人物持旗图与成对方向箭头，支持 A–Z 编解码。";
        ...
        internal static string GetPrinciple(string name)
        {
            switch (name)
            {
                case "格雷码": return "采用二进制反射格雷码：g=b XOR (b>>1)。最高位不变，其余位为相邻二进制位异或；逆转换对格雷码从左到右做累计异或。相邻整数的格雷码只有一位不同。";
                case "进制换算": return "把输入解析为任意精度有理数，再在目标进制逐位展开。有限小数直接输出；能在限制内发现周期时用括号表示循环节；周期过长则保留精确分数。选择截断或最近偶数时会用 ≈ 标记近似值。";
                case "数据类型编码": return "整数采用固定位宽和二进制补码；Float16、Float32、Float64 与 bfloat16 按各自指数、尾数和偏置编码，并使用最近值、遇半取偶；.NET Decimal 使用 96 位有效数、0–28 缩放和符号标志；Q 定点数把值乘以 2 的小数位次幂后量化。字节序只改变多字节位模式的排列。";
                case "碱基转氨基酸": return "使用 NCBI 标准遗传密码表（transl_table=1），每三个碱基对应一个氨基酸。U 与 T 等价；负阅读框先取反向互补链，再按偏移 0、1、2 分组。模糊密码子的所有可能组合若指向同一氨基酸则输出该符号，否则输出 X。来源：https://www.ncbi.nlm.nih.gov/Taxonomy/Utils/wprintgc.cgi#SG1";
                case "旗语": return "每个字母对应两面旗帜在八个方向中的一对位置。人物图与箭头文本共用同一张映射表，方向均按面向观察者的视角绘制。";
                ...
                case "中文语言评分": return "保留汉字后，按模型中的常用字次序和常见连续词组累积得分。自动模式同时计算现代汉语与文言文，采用较高者；样本长度限制显示匹配的上限，匹配不是正确率。";
                case "中文电码加密": return "先把每个汉字转为四位大陆电码，再计算 C=(P+K) mod 10000；解密使用 P=(C−K) mod 10000。破解枚举全部一万个密钥数，联合有效电码覆盖率和所选中文语言模型排列候选。";
                case "反切码": return "从汉字首个普通话读音拆出声母、韵母和声调，以固定代表字记录声母与韵母；解码时重组拼音，再调用内置读音索引列出同音候选。";
                case "中文代码本": return "代码本同时建立“词组→代码”和“代码→词组”索引。加密从当前位置优先取最长词组，避免“中国”被“中”抢先匹配；解密按代码分隔符逐项查表。";
                case "中文隐写分析": return "把文本按 Unicode 字符处理，生成每行固定位置、固定步长和多种方阵路线的候选读法，再以现代汉语或文言文评分并去除重复结果。";
        ...
        internal static string GetUsage(ICryptoTool tool)
        {
            ...
            if (tool.Name == "Book Cipher" || tool.Name == "Nomenclator" || tool.Name == "Running Key" || tool.Name == "VIC" || tool.Name == "中文码表工作台" || tool.Name == "中文代码本") result.Append("   长文本参数可双击输入框或点击 … 打开可缩放编辑器，并可读取文本文件。\r\n");
        ...
        internal static string GetExample(string name)
        {
            switch (name)
            {
                case "格雷码": return "编码：0000 0001 0010 0011 → 0000 0001 0011 0010。解码：0110 → 0100。";
                case "进制换算": return "10→16：255.625 → FF.A；10→2：0.1 → 0.0(0011)；输入 1/3 可得到 0.(3)。十六进制输入可写 0xFF_FF。点击“互换”同时交换源、目标进制。";
                case "数据类型编码": return "Float32、小端：1.0 → 00 00 80 3F；Int32、大端：305419896 → 12 34 56 78；Q16.16、小端：1.5 → 00 80 01 00。互换后按同一类型和字节序解码。";
                case "碱基转氨基酸": return "+1、单字母：ATGGCTTAA → MA*；RNA AUGGCUUAA 结果相同。-1：TTTCAT → MK。三字母格式输出 Met Ala Stop。";
                case "旗语": return "输入 ABC，编码后显示 A、B、C 三个人物持旗图，文本为 ↓↙ / ↓← / ↓↖。点击“互换”可解码回 ABC。";
                ...
                case "中文电码加密": return "输入“中国人民”，密钥数 20，先生成加密后的四位数字组；使用相同密钥解密，或在破解模式自动恢复密钥和原文。";
                case "中文语言评分": return "输入“我们正在处理这个问题”会偏向现代汉语；输入“天地玄黄，宇宙洪荒”会偏向文言文。";
                case "反切码": return "中 → 知翁1；解码“知翁1”会显示 zhong1 及“中”等同音候选字。";
                case "中文代码本": return "代码本填写“中国=88”“中=1”“人民=2”；加密“中国人民”得到 88 2，并因最长匹配不会先使用“中=1”。";
                case "中文隐写分析": return "输入四行分别以“中、国、人、民”开头的文本，选择藏头/藏尾，候选中会出现“中国人民”。";
        ...
        internal static string GetInterpretation(ICryptoTool tool)
        {
            ...
            if (tool.Name == "中文电码加密" || tool.Name == "中文隐写分析" || tool.Name == "中文语言评分")
                return "中文匹配用于比较同批候选，不表示正确概率。短文本可能同时接近现代汉语和文言文；应结合电码有效数、读取路径、上下文和已知片段判断。";
        ...
        internal static string GetTroubleshooting(ICryptoTool tool)
        {
            ...
            if (tool.Name == "中文电码加密") return "电码必须能整理成连续的四位数字组。破解候选较接近时，增加密文长度并明确选择现代汉语或文言文；大陆与台湾电码表不同，码表版本不符时无法得到完整原文。";
            if (tool.Name == "中文代码本") return "每行填写“词组=代码”，也可写“代码 词组”；一项多个代码用斜线分隔。解密出现方括号表示该代码未收录，需导入加密时使用的同一代码本。";
            if (tool.Category == ToolCategories.Encoding || tool.Category == ToolCategories.Chinese)
        ...
        internal static string GetNotes(string name)
        {
            switch (name)
            {
                case "格雷码": return "输入必须是 0/1 位串；空格、制表符和换行均划分独立的数，不是同一个数的分组符。保留位宽，不接受十进制数字或负号。";
                case "进制换算": return "支持 a/b、0.(3)、十进制 1e-3，以及二/八/十六进制的 0b/0o/0x 前缀；下划线只能位于数字之间。小数点和循环节使用点号与半角括号。精确模式不会静默丢失精度；超过小数位上限时改用分数。近似结果带 ≈，不能直接作为下一次输入，需人工确认。每个数最多 4096 字符。";
                case "数据类型编码": return "编码模式用空白分隔多个数；解码模式每行是一条完整位模式，允许空格、下划线、冒号和连字符分隔字节。整数溢出可报错、按模环绕或饱和。Q 定点数可要求精确、截断或最近偶数量化。浮点支持 ±0、NaN、±Infinity；解码默认给出可往返的最短十进制，选择精确可查看真实有理值。Decimal128 指 .NET Decimal 的 canonical flags/high/middle/low 128 位布局，并非 IEEE decimal128。";
                case "碱基转氨基酸": return "输入链按 5′→3′ 书写，一次只接受一条 FASTA。使用标准密码表，不支持线粒体等替代密码表。不会搜索起始密码子，也不应用特殊起始规则；终止密码子输出 * / Stop 并继续翻译。X / Xaa 表示无法唯一确定。默认对不完整末尾报错，也可选择忽略并提示。氨基酸无法唯一逆推碱基。";
                case "旗语": return "图形区可滚动，最多预览前 256 个符号，完整箭头文本仍可复制。支持 A–Z；不支持的图形显示 ?。多文件批量输出仅显示文本。当前没有图片识别或数字模式。";
                ...
                case "中文电码加密": return "该入口实现固定密钥数加减法，适合早期电码加密和快速穷举。自动破解固定尝试 0000–9999，并显示进度。";
                case "中文语言评分": return "现代与文言模型是内置轻量启发式模型，适合候选排序并保持单文件体量。很短的句子、专名、方言和生僻古籍应结合人工判断。";
                case "反切码": return "本工具使用现代普通话声韵拆分和一套稳定代表字。多音字编码采用首个收录读音，解码会保留同音候选；历史韵书可在后续版本作为独立声韵方案加入。";
                case "中文代码本": return "代码本只在当前处理过程中使用。不同词组不要复用同一代码；一词多码可以导入和解码，但加密默认采用该词条列出的第一个代码。";
                case "中文隐写分析": return "自动模式可能生成较多偶然可读片段。行结构会用于藏头、藏尾和同列提取；方阵宽度已知时填写宽度可减少候选。工具不会修改原文。";
