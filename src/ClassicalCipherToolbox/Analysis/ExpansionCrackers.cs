...
namespace ClassicalCipherToolbox.Analysis
{
    internal static class ExpansionCrackers
    {
        ...
        internal static string CrackRunningKey(ToolRequest r)
        {
            ...
            int iterations = Read(r.Get("iterations"), 120000, 1000, 1000000), restarts = Read(r.Get("restarts"), 8, 1, 30); string language = Lang(r); Random random = new Random(4201); List<Candidate> best = new List<Candidate>();
            for (int restart = 0; restart < restarts; restart++)
            {
                char[] key = SeedRunningKey(cipher.Length, restart), topKey = null; for (int i = 0; i < crib.Length; i++) key[i] = (char)('A' + Alphabet.Mod(cipher[i] - crib[i], 26)); string plain = DecodeRunning(cipher, key); double score = JointScore(plain, new string(key), language), top = score; topKey = (char[])key.Clone();
                for (int step = 0; step < iterations; step++) { if ((step & 255) == 0) { r.ThrowIfCancellationRequested(); r.ReportProgress((restart * iterations + step) * 100 / (restarts * iterations), "Running Key · 重启 " + (restart + 1)); } int p = crib.Length >= key.Length ? key.Length - 1 : crib.Length + random.Next(key.Length - crib.Length); char old = key[p]; key[p] = (char)('A' + random.Next(26)); string trial = DecodeRunning(cipher, key); double value = JointScore(trial, new string(key), language), temp = 14 * (1.0 - step / (double)iterations) + .2; if (value >= score || random.NextDouble() < Math.Exp((value - score) / temp)) { score = value; plain = trial; if (value > top) { top = value; topKey = (char[])key.Clone(); } } else key[p] = old; }
